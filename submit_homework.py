...
def submit(subject,homework_title,group_No):
    ...
    if flag == 235:
        ...
        # 查找qid list
        cur.execute(
            "SELECT hr.Content FROM ETeacher_Homework_Resource hr  WHERE hr.Title = " + "'" + homework_title + "'")

        qid_list = eval(cur.fetchall()[0][0])
        # 查找groupid
        cur.execute(
            "SELECT gh.GroupID FROM	ETeacher_Group_HomeworkGroup gh JOIN ETeacher_Homework_Resource hr ON gh.HomeworkID = hr.HomeworkID JOIN ETeacher_Group g ON g.ID = gh.GroupID WHERE hr.Title = " + "'" + homework_title + "'" + " AND g.GroupNo = " + str(
                group_No))
        group_id = cur.fetchall()[0][0]
        # 查找homeworkid
        cur.execute(
            "SELECT gh.HomeworkID FROM	ETeacher_Group_HomeworkGroup gh JOIN ETeacher_Homework_Resource hr ON gh.HomeworkID = hr.HomeworkID JOIN ETeacher_Group g ON g.ID = gh.GroupID WHERE hr.Title = " + "'" + homework_title + "'" + " AND g.GroupNo = " + str(
                group_No))
        homework_id = cur.fetchall()[0][0]

        # 查找userid list
        cur.execute(
            "SELECT gs.UserID FROM ETeacher_Group_StudentGroup gsg JOIN ETeacher_Group_Student gs ON gsg.StudentID = gs.ID  WHERE gsg.GroupID = " + str(
                group_id))
        userid_list = cur.fetchall()
        cur.close()
        conn.close()
        ...
        for username in username_list:
            ...
            t = re.findall("(?<=user=tk=).*?(?=;)", str(requests.get(
                'http://my.235.mistong.com/login/prelogin?sid=2&username=' + username + '&password=123456').headers))[0]
            print username, requests.post('http://teacher.235.mistong.com/student/myhomework/submitexam',
                                          data='g=' + str(group_id) + '&h=' + str(
                                              homework_id) + '&a=' + answer + '&t=00%3A00%3A16',
                                          headers={'Content-Type': 'application/x-www-form-urlencoded',
                                                   'cookie': 'user=tk=' + t}).content


submit('ShuXue','567',468290)
